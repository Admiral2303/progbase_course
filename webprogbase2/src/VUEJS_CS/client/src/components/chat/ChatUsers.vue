...
<div>
        ...
        <div class="col-md-1 col-md-offset-0">
            ...
            <div v-for="User of users" :key="User._id">
                <button @click="getmessages(User._id)"><img :src="User.image" alt="<%= User.name %>" class="img"></td></button>
                {{User.name}}
                <hr color="black">
            </div>
        </div>   
...
export default {
    ...
    async mounted(){
        let response =  await ChatService.chatusers();
    ...
    methods:{
        async getmessages(id){
            ...
            this.$store.dispatch('setChatUserid',  chat_obj.data.receiver._id);
