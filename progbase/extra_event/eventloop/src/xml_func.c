...
void XmlLoader_saveToFile(struct Freelancer * self, char * filename){
	...
	xmlNewChild(freelancerNode, NULL, BAD_CAST "score", BAD_CAST strBuf);
		...
		//puts((const char *)bufferPtr->content);
...
void XmlLoader_loadConfiguration(Configuration * self, char * filename) {
    ...
    for(xmlNode * xCur = xRootEl->children; NULL != xCur ; xCur = xCur->next) {
        if (XML_ELEMENT_NODE == xCur->type) {
            for (xmlNode * xJ = xCur->children; NULL != xJ ; xJ = xJ->next) {
					char * content = (char *)xmlNodeGetContent(xJ);
					if(xmlStrcmp(xJ->name, BAD_CAST "filepath") == 0){
...
void Save_configuration(char * filename, Configuration * self){
	...
	xmlNode * freelancerNode = NULL;
	doc = xmlNewDoc(BAD_CAST "1.0");
	rootNode = xmlNewNode(NULL, BAD_CAST "configurations");
	xmlDocSetRootElement(doc, rootNode);
	freelancerNode = xmlNewChild(rootNode, NULL, BAD_CAST "configuration", NULL);
	xmlNewChild(freelancerNode, NULL, BAD_CAST "filepath", BAD_CAST self->filepath);
	xmlBuffer * bufferPtr = xmlBufferCreate();
	xmlNodeDump(bufferPtr, NULL, (xmlNode *)doc, 0, 1);
